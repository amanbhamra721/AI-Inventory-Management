...
def process_image_task(media_id: str, sender_no: str):
    ...
    try:
        ...
        if result:
            ...
            try:
                ok, db_message = insert_transaction(result, sender_phone=sender_no)
                if not ok:
                    logger.warning(f"⚠️ DB Insert Skipped/Blocked: {db_message}")
                    if "Duplicate blocked" in db_message:
                        send_whatsapp_text(sender_no, "⚠️ Duplicate receipt detected. Entry was not saved.")
                    elif "Queued for review" in db_message:
                        send_whatsapp_text(sender_no, "🟡 Receipt queued for manual review due to low AI confidence.")
                    else:
                        send_whatsapp_text(sender_no, f"⚠️ Could not save receipt: {db_message}")
                    return
                logger.info(f"✅ Success! Transaction from {sender_no} saved to DB. {db_message}")
            except Exception:
